...
export default async function CodingInterface({ session, id, params }) {
  ...
  const fetchDataForAssignment = useCallback(async () => {
    console.log("Fetching assignment data...", assignment_id);
    ...
    try {
      const data = await getAssignmentDetails(assignment_id);
  ...
  }, [fetchDataForAssignment, assignment_id]);
